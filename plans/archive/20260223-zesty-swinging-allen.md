...
| 내 카드 | 이미지 (jpg) | 아이콘 기반 카드 (컬러 원형 + lucide 아이콘 + 텍스트) |
| 상대 카드 | 이모지 카드 | FaceDownCard 컴포넌트 (그라디언트 + shield 아이콘) |
...
- FaceDownCard: 그라디언트(#1A1A1A→#2A2A2A) + Shield 아이콘 + "COUP" 텍스트
- 공개된 카드: CharacterCard (opacity 40%) + "Eliminated" 텍스트
- 카드 크기: 80×112px
...
- 이미지 기반 → 아이콘 기반 CharacterCard로 변경
- CharacterCard: 컬러 원형(48px) + Lucide 아이콘 + 캐릭터명 + 능력 설명
- 카드 크기: 120×170px
