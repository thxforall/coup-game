...
Next.js 14 + Firebase Realtime Database + Tailwind CSS로 만든 **실시간 멀티플레이어 쿠(Coup) 보드게임**입니다. 2~10인이 각자의 디바이스에서 접속하여 블러핑, 도전, 쿠데타를 즐길 수 있습니다.
...
### 🎮 게임 플레이
- **2~10인 실시간 멀티플레이** — Firebase Realtime Database 기반 동기화 (기본 6인, 종교개혁 모드 10인)
- **완전한 쿠 규칙 구현** — 소득, 외국 원조, 세금징수, 암살, 갈취, 교환, 쿠데타
- **도전 & 블록 시스템** — 모든 플레이어가 도전/블록/패스 선택 가능
- **3가지 게임 모드** — Standard(기본), Guess(추측), Reformation(종교개혁 확장판)
- **종교개혁 확장판** — 진영(충성파/개혁파), 전향, 횡령, 인퀴지터(심문), 재무부 시스템
- **30초 응답 타이머 + 45초 턴 타이머** — 시간 초과 시 자동 패스/소득/쿠데타 처리
- **랜덤 선공** — 매 게임 시작 시 무작위로 첫 번째 플레이어 결정
- **로비 방 목록 브라우저** — 대기 중/진행 중 방 실시간 조회, 상태 배지 표시
...
| 🔍 **종교재판관** (Inquisitor) | 교환(1장) + 심문 — 상대 카드 확인 후 교체 강제 | 갈취 차단 |

> 💡 종교재판관은 **종교개혁 모드**에서 대사를 대체합니다.

### 종교개혁 모드 전용 액션

| 액션 | 효과 | 비용 |
|------|------|------|
| 🔄 **전향** | 자기/타인 진영 변경 | 자기 1코인 / 타인 2코인 → 재무부 |
| 💰 **횡령** | 재무부 코인 전액 획득 (공작 능력) | 없음 |
| 🔍 **심문** | 상대 카드 확인 → 돌려주기 or 교체 강제 (인퀴지터 능력) | 없음 |
...
| 버전 | 날짜 | 요약 |
|------|------|------|
| [v0.6.0](./CHANGELOG.md#060--2026-02-24) | 2026-02-24 | 종교개혁 확장판(진영/인퀴지터/전향/횡령/심문), 방 목록 진행중 표시, ExchangeModal 개선, 타이머/모달 UX |
