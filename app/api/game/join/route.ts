...
import { supabase, getRoom, updateRoom } from '@/lib/supabase';
...
export async function POST(req: NextRequest) {
  ...
  if (state.players.length >= 6) {
    return NextResponse.json({ error: '방이 가득 찼습니다 (최대 6명)' }, { status: 400 });
  }
  ...
  await updateRoom(roomId.toUpperCase(), updatedState);
